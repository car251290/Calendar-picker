...
class TableColumns {
    constructor($table, tableName) {
      ...
      this.listOfHidden = [];
      // change by JC to use sort method
      // sort the columns with the first column as default
      this.sortOrder = {};
      ...
      if (this.$rows[0]) {
        //update to Array from that is the same as slice allowing the pattern bein used to convert this array
      ...
      // Set up toggle menu
      this.createControls();
      // the toggle menu is created now we can add sorting
      this.addSortings();
    ...
    // function to add soring event listeners
    addSortings(){
      this.$headers.forEach((header,index)=> {
        if (!this.protectedCols.includes(index)) { // Skip protected columns
          header.classList.add('sortable'); // Optional: add a class for styling
          header.addEventListener('click', () => this.sortByColumn(index));
        }
      });
    }
    // function to sort by given column change the JC to use sort method
    sortByColumn(columnIndex) {
      const ascending = !this.sortOrder[columnIndex]; // Toggle sort order
      this.sortOrder[columnIndex] = ascending;
  
  
      // Sort rows change by JC to use sort method
      ...
      this.$headers.forEach(header => header.classList.remove('asc', 'desc'));
      this.$headers[columnIndex].classList.add(ascending ? 'asc' : 'desc');
    }
    // Existing method: Toggles a column's visibility
    toggleColumn(index, hide) {
      this.$rows.forEach(row => {
        row.children[index].style.display = hide ? 'none' : '';
      });
      this.$headers[index].style.display = hide ? 'none' : '';
    }
    // end of the change by JC
